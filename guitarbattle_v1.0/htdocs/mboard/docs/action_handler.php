<?php
	...
	switch($mode) {
		case('post_message'): {
			if(!$login) {
			...
			else if((strlen($post_subject) < 1) || (strlen($post_body) < 1)) {
				...
				mysql_query("insert into $board_table (user, user_id, subject, body, timestamp, parent) values('$current_user', $guitar_battle_login, '$post_subject_fix', '$post_body_fix', '$timestamp', $parent_post)");
				if($parent_post == 0) {
