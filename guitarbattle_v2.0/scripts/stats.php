...
<?
	...
	if(!($res = mysql_query("select count(*) as count from mp3s"))) {
		print "MySQL Error: " . mysql_error($db) . "\n";
		exit;
	}
	$num_mp3s = mysql_result($res, 'count', 0);

	if(!($res = mysql_query("select sum(length(file_contents)) as size from mp3s"))) {
		print "MySQL Error: " . mysql_error($db) . "\n";
		exit;
	}
	...
	if(!($res = mysql_query("select count(*) as count from battles where winner='' and user2!=''"))) {
		print "MySQL Error: " . mysql_error($db) . "\n";
		exit;
	...
	if(!($res = mysql_query("select count(*) as count from battles where winner!=''"))) {
		print "MySQL Error: " . mysql_error($db) . "\n";
		exit;
	...
	print "There are " . $num_hidden_battles . " hidden battles.\n";
